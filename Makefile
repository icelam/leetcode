...
new-readme-entry:
	$(if $(strip $(ID)), $(eval FILE_PATH=$(shell find ${JAVA_SOURCE_DIRECTORY} ${TYPESCRIPT_SOURCE_DIRECTORY} ${MYSQL_SOURCE_DIRECTORY} ${BASH_SOURCE_DIRECTORY} -name *${ID}* | tr '\n' ',')))
	...
	readme_entry=f"| {int(problem_id)}. {problem_name} | {level} | {encoded_java_path} | {encoded_typescript_path} | {encoded_mysql_path} | {encoded_bash_path} |"; \
